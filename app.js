...
// an array of strings
// separate individual string-element into array -> people[i].split(", ");
// output = an array of array
// swap position of the element in nested array -> people[i][0], people[i][1] = people[i][1], people[i][0]
// combine element of individual array  -> people[i].join(" ")

// people.forEach((person) => {
//   let newPerson = "";
//   newPerson = person.split(", ");
//   [newPerson[0], newPerson[1]] = [newPerson[1], newPerson[0]];
//   newPerson = newPerson.join(" ");
//   //console.log(newPerson);
//   firstLast.push(newPerson); 
// });


let firstLast = [];
firstLast = people.map((person)=>{
    let newPerson= person.split(", ");
    return `${newPerson[1]} ${newPerson[0]}`; 
});
